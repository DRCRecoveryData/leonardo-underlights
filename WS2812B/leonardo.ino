...
void handleNoteOn(byte channel, byte pitch, byte velocity) {
  if (pitch >= _pStart && pitch < _pStart + _nLED) {
  ...
  do {
    ...
    if (rx.header != 0) {
      switch (rx.byte1 & 0xF0) {
        case 0x90: // note on
          handleNoteOn(rx.byte1 & 0x0F, rx.byte2, rx.byte3);
          break;
        case 0x80: // note off
          handleNoteOff(rx.byte1 & 0x0F, rx.byte2, rx.byte3);
          break;
        case 0xF0: // sysEx
          byte data[3] = {rx.byte1, rx.byte2, rx.byte3};
          handleSysEx(data, 3);
          break;
      }
    }
